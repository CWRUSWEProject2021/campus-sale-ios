    ...
    var body: some View {
        VStack() {
            ...
            if #available(iOS 15.0, *) {
                //List((items.filter({ searchText.isEmpty ? true : $0.keywords.contains(searchText) || $0.name.contains(searchText)})))
                
                List((items.filter({ searchText.isEmpty ? true : $0.keywords.localizedCaseInsensitiveContains(searchText) || $0.name.localizedCaseInsensitiveContains(searchText)}))) { item in
